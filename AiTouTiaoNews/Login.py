    ...
    for i in codelist:
        ...
        if d(resourceId="com.tencent.reading:id/mine_tab_better_header_user_name").exists:
            ...
            d(resourceId="com.tencent.reading:id/send_btn",
              text="重新发送").click_exists(timeout=10)
...
def check_Huya_login_stats(d):
    if d(resourceId="com.duowan.kiwi:id/name",
...
def detect_JinriNews_app_start(d):
    try:
        if d(
            resourceId="com.duowan.kiwi:id/title",
            className="android.widget.TextView",
                text="首页").exists:
            d(resourceId="com.duowan.kiwi:id/title",
              className="android.widget.TextView",
              text="首页").click_exists(timeout=3)
...
def register(d, Login_number):
    ...
    if d(resourceId="com.duowan.kiwi:id/title", text=u"我的").exists:
        d(resourceId="com.duowan.kiwi:id/title",
          text=u"我的").click_exists(timeout=3)
    else:
    ...
    if d(resourceId="com.duowan.kiwi:id/login_button", text="立即登录").exists:
        d(resourceId="com.duowan.kiwi:id/login_button",
          text="立即登录").click_exists(timeout=3)
        d(resourceId="com.duowan.kiwi:id/user_name",
          text="通行证/YY号/手机号码/邮箱").clear_text()
        if Login_number["Type"] == "Phone":
            d(resourceId="com.duowan.kiwi:id/user_name",
              text="通行证/YY号/手机号码/邮箱").set_text(Login_number["Phone"])
            #d(resourceId="com.tencent.reading:id/send_btn", text="获取验证码").click_exists(timeout=10)
            d(resourceId="com.duowan.kiwi:id/password",
              className="android.widget.EditText").set_text(Login_number["Code"])
            d(resourceId="com.duowan.kiwi:id/login",
              text="登录").click_exists(timeout=3)
        elif Login_number["Type"] == "YY_number":
            d(resourceId="com.duowan.kiwi:id/user_name",
              text="通行证/YY号/手机号码/邮箱").set_text(Login_number["YY_number"])
            # d(resourceId="com.tencent.reading:id/send_btn", text="获取验证码").click_exists(timeout=10)
            d(resourceId="com.duowan.kiwi:id/password",
              className="android.widget.EditText").set_text(Login_number["Code"])
            d(resourceId="com.duowan.kiwi:id/login",
              text="登录").click_exists(timeout=3)
        elif Login_number["Type"] == "QQ":
            d(resourceId="com.ss.android.article.news:id/bz").click_exists(timeout=3)
            if d(
                resourceId="com.tencent.mobileqq:id/name",
                    text=u"授权并登录").exists:
                d(resourceId="com.tencent.mobileqq:id/name",
                  text=u"授权并登录").click_exists(timeout=3)
            else:
                print("QQ方式登录失败")

        elif Login_number["Type"] == "Wechat":
            d(resourceId="com.ss.android.article.news:id/c5").click_exists(timeout=3)
            if d(resourceId="com.tencent.mm:id/ch9", text=u"确认登录").exists:
                d(resourceId="com.tencent.mm:id/ch9",
                  text=u"确认登录").click_exists(timeout=3)
            else:
                print("Wechat方式登录失败")
        else:
            d(resourceId="com.duowan.kiwi:id/user_name",
        ...
        d(resourceId="com.duowan.kiwi:id/button_positive",
          text="确定退出").click_exists(timeout=3)

    if login_stat == 1:
        print("登录失败，请人为手动重新登录.")
        d(resourceId="com.ss.android.article.news:id/cng",
          text=u"首页").click_exists(timeout=2)
    else:
        print("登录成功")
        d(resourceId="com.ss.android.article.news:id/cng",
          text=u"首页").click_exists(timeout=2)
